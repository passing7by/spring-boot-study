...
SELECT p.PLAYER_NAME 선수명, p."POSITION" 포지션, p.NATION 출신지, t.TEAM_NAME 팀명, s.STADIUM_NAME 홈경기장명
FROM PLAYER p 
RIGHT JOIN TEAM t USING (TEAM_ID)
RIGHT JOIN STADIUM s USING (STADIUM_ID);

--9. TEAM, STADIUM 테이블을 활용하여 각 팀의 이름과 경기장ID, 경기장명을 조회하되
--   경기장ID가 존재하는 팀만 조회 하고 결과는 경기장 명이 오름차순 정렬이 되도록 조회하시오.
SELECT t.TEAM_NAME 팀명, s.STADIUM_ID 경기장ID, s.STADIUM_NAME 경기장명
FROM TEAM t
RIGHT JOIN STADIUM s ON t.STADIUM_ID = s.STADIUM_ID
ORDER BY 경기장명;

--10. PLAYER 테이블을 활용하여 신장 크기가 모든 선수의 신장 길이의 평균 이상인 선수들의 선수명, 포지션, 등번호를 
-- 선수이름 기준 오름차순으로 조회하시오.
SELECT p.PLAYER_NAME 선수명, p."POSITION" 포지션, p.BACK_NO 등번호
FROM PLAYER p
WHERE p.HEIGHT
	  >= (SELECT AVG(p2.HEIGHT) FROM PLAYER p2)	
ORDER BY 선수명;

--11. 선수 중 '정현수'라는 동명이인이 속한 팀의 한글 명칭과 영문 명칭, 소속 지역을 조회하시오.
SELECT t.TEAM_NAME 한글이름, t.E_TEAM_NAME 영문이름, t.REGION_NAME 소속지역
FROM PLAYER p
...
--14. SCHEDULE에 기록된 정보들 중 HOME팀과 AWAY팀의 합산 점수가 가장 높은 경기의 
